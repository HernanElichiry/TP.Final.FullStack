...
export const ProductCard = ({
  ...
  const handleCardClick = () => {
    //funcion para manejar el redireccionamiento a la couseDetail al hacer click
    ...
    const fetchImage = async () => {
      try {
        const imageResponse = await fetch(
          `http://localhost:3000/uploads/images/${product.media.filename}`,
          {
            method: "GET",
            mode: "cors",
            credentials: "include",
  ...
  return (
    <div className="card-product-container">
      ...
      {/* en vez de mostrar la valoracion uso un metodo que muestra una estrella por cada punto*/}
      <div className="price-favorite-container">
        {product.price == 0 ? <p>Gratis</p> : <p>Precio: ${product.price}</p>}
      </div>
      <div className="buttons">
        <button className="show-more-button" onClick={handleCardClick}>
          Show More
        </button>
